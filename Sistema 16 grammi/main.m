...
eta=0.75; 
Na=6.022*1e23; %[mol^-1] Numero di Avogadro
...
sigma=5.67*1e-8; %[W/(m^2*K^4)] costante di Stefan-Boltzmann
...
D_water=0.01; %[m]
mu_water=1e-3;%[Pa*s]
cp_water=4184.4; %[J/(kg*K)]
Pr=mu_water*cp_water/k_water;
Re_water=rho_water*v_water*D_water/mu_water;
Nu_water=0.023*Re_water^(4/5)*Pr^0.4;
hh_w=Nu_water*k_water/D_water; % [W/(m^2*K)]
...
% Concentrazione di litio nelle camere
Ncold=0; % [#/m^3] centrale
Nsold=0; % [#/m^3] sinistra
Ndold=0; % [#/m^3] destra
...
legend('3000s', '3500s', '4000s','4230s')
grid minor
